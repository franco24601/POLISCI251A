...
```{r}
### Use the function print and your model
print(lasso)
```
...
coef_lasso <- coef(lasso,s=0.023530)
...
# plot MSE x lamda
plot(store_mse_in~lasso$lambda, xlab="Lambda", ylab="MSE in-sample",type="l")
```
...
```{r}
min_lambda<- lasso$lambda[which.min(store_mse_in)]
min_lambda
```
...
coef_lasso <- coef(lasso,s=min_lambda)
# Let's print the non-zero coefficients
## Fron this object, select only those that are non zero
coef_lasso <- round(coef_lasso[which(coef_lasso[,] !=0),] , 4)
## Sort them from large to small
...
```{r}
lambda_min_out <- lasso_cv$lambda[which.min(out_of_sample)]
lambda_min_out
```
...
```{r}
plot(lasso_cv$cvm ~ lasso_cv$lambda, xlab="Lambda", ylab="CV error", type="l") + 
  abline(v=lasso_cv$lambda.min, lty="dashed", col="red" )
...
```{r}
plot(out_of_sample ~ lasso_cv$lambda, xlab="Lambda", ylab="Test error", type="l") + 
  abline(v=lambda_min_out, lty="dashed", col="red" )
```
