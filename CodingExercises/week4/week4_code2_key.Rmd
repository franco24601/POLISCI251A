...
for(i in 1:nrow(elect)){
  ...
  model <- lm(IncumbentVoteShare~ Q1_GDP_Growth+Q2_GDP_Growth+
                Incumbent_Net_Approval+ Incumbent_Party_Two_Terms, data=train)
  ## Predict using this model in test
  pred  <- predict(model, newdata=test)
  ### Calculate the squared error. Error= vote share for row i minus prediction
  sqr_error <- (test$IncumbentVoteShare- pred)^2
  #### Store in the position i of errors_loocv
  errors_loocv[i]  <- sqr_error
}
