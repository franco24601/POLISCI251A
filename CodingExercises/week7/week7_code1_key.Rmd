...
```{r}
setwd('C://Users/franc/Dropbox/PhD/5_year/summer/ML_class/Lectures/Lecture11/')
...
Look up the help page for the `sapply` function. 
```{r eval=FALSE}
?sapply
```


Using this function, calculate the closest document for each email.
...
cos.sim <- lsa::cosine(flake_matrix_t)
```
